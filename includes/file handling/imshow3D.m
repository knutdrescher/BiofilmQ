function  imshow3D( Img, disprange, Img2 )
...
if nargin == 1
    axes('position',[0,0.2,1,0.8]), imagesc(Img(:,:,S))
else
    Img = Img-Rmin;
    Img = Img/(Rmax-Rmin);
    ...
    axes('position',[0,0.2,1,0.8]), imshow(Img_plot, [0 1])
    hold on
    
    if nargin == 3
        objects = 255-label2rgb(Img2(:,:,S));
        
        h = imshow(objects, [0 1]);
        hold off;
        set(h, 'AlphaData', 0.1*ones(size(Img_plot)));
    end
end
...
% -=< Mouse scroll wheel callback function >=-
    function mouseScroll (object, eventdata)
        ...
        if length(ch) > 1
            set(ch(2),'cdata',Img(:,:,S))
            objects = 255-label2rgb(Img2(:,:,S));
            set(ch(1),'cdata',objects)
        else
            set(ch(1),'cdata',Img(:,:,S))
        end
    end
