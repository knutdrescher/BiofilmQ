function mvtk_write(M,filename,format,custom_fields)
% modified by Raimo Hartmann
...
switch lower(format)
    case {'legacy','legacy-ascii'}
        mvtk_write_legacy(M,filename,'ASCII',custom_fields);
    case {'legacy-binary'}
        mvtk_write_legacy(M,filename,'BINARY',custom_fields);
    case {'xml','xml-ascii'}
        mvtk_write_xml(M,filename,'ASCII',custom_fields);
    case {'xml-binary'}
        mvtk_write_xml(M,filename,'BINARY',custom_fields);
    case {'xml-appended'}
        mvtk_write_xml(M,filename,'APPENDED',custom_fields);
    otherwise
        error('Unknown file format.');
end
...
%==========================================================================
function fid = mvtk_write_legacy(s,filename,format,custom_fields)
...
for i = 1:length(custom_fields)
    ...
    dataName = custom_fields{i};
    fprintf(fid,'SCALARS %s %s %d\n',dataName,'float',size(s.(custom_fields{i}),2));
    fprintf(fid,'LOOKUP_TABLE %s\n',lut_name);
    fmt = repmat('%f ',1,size(s.(custom_fields{i}),2)); fmt(end) = '';
    write_data(fid,[fmt '\n'],'float32',s.(custom_fields{i})');
    if ~strcmp(lut_name,'default')
        fprintf(fid,'LOOKUP_TABLE %s %d\n',lut_name,size(s.lut,1));
        if strcmp(format,'ASCII')
            % float values between (0,1)
...
%==========================================================================
function fid = mvtk_write_xml(s,filename,format,custom_fields)
...
for i = 1:length(custom_fields)
    ...
    if isfield(s,custom_fields{i}) && ~isempty(s.(custom_fields{i}))
        [offset,dat] = write_data('%f ',single(s.(custom_fields{i})'));
        DataArray = struct;
        DataArray.type = 'Float32';
        DataArray.Name = 'scalars';
        DataArray.NumberOfComponents = sprintf('%d',size(s.(custom_fields{i}),2));
        DataArray.format = format;
        if ~isnan(offset), DataArray.offset = sprintf('%d',offset); end
        fprintf(fid,'%s<DataArray',o(4));
        for i=fieldnames(DataArray)'
